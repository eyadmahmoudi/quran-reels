...
RULE 2 — EXACT PHRASE PREFERENCE (VERY IMPORTANT):
The candidate verses are sorted by relevance — verses listed FIRST are the BEST matches. You MUST prefer the FIRST-listed verses that match the user's exact phrase. For example:
- If the user asks "من قال إنهم عدو لي", the FIRST candidate containing "عدو لي" is the answer (26:77 - Abraham). Do NOT pick a verse that has "عدو لكم" or "عدوا لجبريل" instead — the exact phrase "عدو لي" is what matters.
- The candidate verses are pre-ranked so that exact phrase matches appear FIRST. Trust the ordering.
...
export async function POST(req: NextRequest) {
  ...
  // BONUS: If a verse contains the original question as a substring,
  // give it a massive score boost (exact phrase match).
  const qBaseNorm = normalizeBase(question)
  const qMorphoNorm = normalizeMorpho(question)

  for (const list of lexicalLists) {
    for (const h of list) {
      // Exact phrase bonus: check if the verse contains the full question text
      let bonusScore = h.score
      const verseBase = normalizeBase(h.text)
      const verseMorpho = normalizeMorpho(h.text)
      if (qBaseNorm.length >= 4 && verseBase.includes(qBaseNorm)) {
        bonusScore += 10  // Massive boost for exact phrase match in strict tier
      } else if (qMorphoNorm.length >= 4 && verseMorpho.includes(qMorphoNorm)) {
        bonusScore += 8   // Strong boost for morphological phrase match
      }
      ...
      merged.push({ vk: h.verse_key, ar: stripMark(h.text), tr: '', score: bonusScore })
    }
  }
