...
export function MainStage() {
  ...
  return (
    <main className="relative flex flex-1 flex-col items-center justify-center overflow-hidden bg-slate-50 px-4 py-6 dark:bg-zinc-950 sm:px-6 sm:py-8">
      {/* Ambient accent glow */}
      <div
      ...
      <div className="relative mx-auto flex w-full max-w-[280px] items-center justify-center sm:max-w-[320px] lg:h-full lg:max-w-[min(100%,calc(100vh*0.5625))]">
        <div className="relative aspect-[9/16] w-full lg:h-full lg:max-h-full lg:w-auto">
          <div className="relative h-full w-full overflow-hidden rounded-2xl border border-zinc-200 bg-black shadow-2xl shadow-emerald-900/10 dark:border-zinc-800 dark:shadow-emerald-900/20">
            {showPreview ? (
              <PreviewCanvas
          ...
          <div className="mt-3 flex items-center justify-between text-[11px] text-zinc-500 dark:text-zinc-500">
            ...
            {hasSurah ? (
              ...
              <span className="text-zinc-400 dark:text-zinc-600">No surah selected</span>
            )}
          </div>

          {error && (
            <div className="absolute -bottom-12 left-0 right-0 mx-auto flex w-fit items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-xs text-red-600 dark:text-red-400">
